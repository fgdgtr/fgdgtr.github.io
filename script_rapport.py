    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if isinstance(value, dict):
                # DEBUG : affiche les clés disponibles pour la première palette
                # print(f"Clés disponibles: {list(value.keys())}")
                palettes.append({
...
def get_emplacements(palettes_list, n=3):
    """Retourne les n premiers emplacements valides (non N/A, non vide)."""
    valides = []
    for p in palettes_list:
        if isinstance(p, tuple):
            e = p[0].get('emplacement', '')
        else:
            e = p.get('emplacement', '')
        if e and e != 'N/A':
            valides.append(e)
    valides = valides[:n]
    return ' | '.join(valides) if valides else '-'
...
def generate_pdf(stats, filename='rapport_rack.pdf'):
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle('heading', parent=styles['Heading2'], fontSize=12, fontName='Helvetica-Bold')
    ))
    story.append(Spacer(1, 0.2*cm))

    # FIX : utilisation de Paragraph pour le rendu bold au lieu de balises HTML brutes
    bold_style = ParagraphStyle(
        'bold_val', parent=styles['Normal'],
        fontSize=12, fontName='Helvetica-Bold', alignment=TA_RIGHT
    )

    movements_data = [
        ['Palettes arrivées cette semaine', Paragraph(str(stats['arrivees_semaine']), bold_style)],
        ['Total palettes en attente',       Paragraph(str(stats['total']), bold_style)],
        ['Palettes urgentes (+28j)',         Paragraph(str(stats['par_zone']['plus_28']), bold_style)],
    ]
    ...
    # FIX : utilisation de get_emplacements() pour filtrer les N/A
    zone_data = [
        ...
        ['🔴 +28 jours',
         ...
         (' ⚠️ DÉLAI DÉPASSÉ' if stats['details_zone']['plus_28'] else '')],
    ]
    ...
    if stats['details_urgentes']:
        ...
        story.append(Paragraph(
            "🚨 PALETTES URGENTES À TRAITER",
            ParagraphStyle('heading_red', parent=styles['Heading2'], fontSize=12,
        ...
        for palette, jours in stats['details_urgentes']:
            delai_dep = jours - 28
            emplacement = palette.get('emplacement', '')
            client = palette.get('client', 'N/A')
            # Affiche N/A uniquement si vraiment vide
            emplacement_display = emplacement if emplacement and emplacement != 'N/A' else '—'
            urgent_data.append([
                emplacement_display,
                client[:15] if client != 'N/A' else '—',
                str(jours),
                f"+{delai_dep}j"
            ])
...
def main():
    ...
    # DEBUG : décommente pour voir les clés Firebase si les emplacements sont N/A
    # if palettes:
    #     print(f"Exemple de palette: {palettes[0]}")
